...
export async function POST(req:NextRequest,context:{params:Promise<{orderId:string}>;}) {
    try {
        ...
        if(status==="out for delivery" && !order.assignment){
            ...
            console.log("Searching near:", [longitude, latitude]);
            const allBoys = await User.find({ role: "deliveryBoy" });
            console.log("Total delivery boys in DB:", allBoys.length);
            ...
            if(candidates.length==0){
                ...
                return NextResponse.json(
                    {message : "there is no delivery boy" },
                    {status : 200}
                )
            }
        ...
        return NextResponse.json(
            {messgae:`update status error ${error}`},
            {status:500 }
        )
    }
}