...
<div className='w-full flex items-center justify-center gap-3 border border-gray-300 hover:border-gray-50 py-3 rounded-3xl text-gray-400 font-medium transition-all duration-200 cursor-pointer' onClick={()=> signIn("google",{callbackUrl:"/"})}>
    ...
    Continue with Google
        ...
        <p className='cursor-pointer text-gray-600 mt-6 text-sm flex items-center gap-1' onClick={()=> router.push("/register")}>
            Create an account ? <LogIn className='w-4 h-4'/> <span className='text-green-600'>Sign Up</span>
        </p>
    </div>
  )
}
