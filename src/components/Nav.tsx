  ...
  const sideBar = menuOpen?createPortal(
    <AnimatePresence>
      ...
      >
        ...
        <div className="flex items-center gap-3 p-3 mt-3 rounded-xl bg-white/10 hover:bg-white/20 transition-all shadow-inner">
        ...
        <div className="flex items-center gap-3 font-semibold text-red-300 mt-auto hover:bg-red-500/20 p-3 rounded-lg transition-all cursor-pointer" onClick={async ()=>await signOut({callbackUrl :"/"})}><LogOut className="w-5 h-5 text-red-300" />Logout</div>
      </motion.div>
  ...
  return (
    <div className="w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50">
      ...
      <div className="flex items-center gap-3 md:gap-6 relative">
        ...
        {user.role=="admin" && <>
        <div className="hidden md:flex itemes-center gap-4">
            ...
            {searchBarOpen && (
              ...
              >
                ...
                <form className="grow" onSubmit={handleSearch}>
                  <input value={search}
                  onChange={(e)=>setSearch(e.target.value)} type="text" placeholder="Search Product" className="w-full outline-none text-gray-700 "/>
                </form>
